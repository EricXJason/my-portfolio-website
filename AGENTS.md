...
> *Version: 2.4.0 | Last Updated: 2026-07-29*

![Author: HSU, CHE-CHENG](#)
![Version: 2.4.0](#)
![Standard: Clean Code](#)
![Compliance: Strict IP](#)
![Agent: Ready](#)
...
> **MANDATORY PRESENCE GATE**: The 3 core dynamic documentation files (`docs/check-list.md`, `docs/system-spec.md`, `docs/change-log.md`) ARE MANDATORY FOR EVERY SINGLE PROJECT TYPE WITHOUT EXCEPTION. Upon task completion, verify all 3 files physically exist in `docs/` with fully populated, valid content.
> **DEVELOPMENT VS. PRE-GIT-ADD LANGUAGE MATRIX (TOKEN OPTIMIZED)**:
> - **In-Development Phase (Unstaged)**: `docs/check-list.md` MUST contain Traditional Chinese at minimum for human review. `docs/system-spec.md` and `docs/change-log.md` MAY remain in professional English during active coding to maximize execution speed and minimize Token usage.
> - **Pre-Git-Add Phase (Before Staging)**: Prior to ANY `git add` or staging action, BOTH the root `README.md` AND `docs/system-spec.md` MUST be fully updated/generated in BILINGUAL format (English & Traditional Chinese) with strict semantic translation accuracy.
...
| :--- | :--- | :--- |
| **`docs/check-list.md`** | **Initial Step of Task Execution**: Immediately upon receiving ANY new user prompt | 1. Parse and append new requirement items marked as `[X]` (Must include Traditional Chinese) BEFORE writing code.<br>2. **Prune Accepted Items**: Automatically REMOVE all human-approved `[O]` items from the file. Retain ONLY active `[X]` items.<br>3. **Git Isolation**: Local acceptance tool ONLY. MUST be added to `.gitignore` and NEVER pushed to remote Git repositories. |
| **`docs/system-spec.md`** | Project init, major bump, DB migration, API restructuring, or workflow change | Integrated Master Spec: Consolidated document containing Overview, Core Features, Architecture/Data Flow, and Tech Stack. (Dev: English; Pre-Git-Add: Bilingual). |
...
2. **Early Task Initialization & Pruning**: At the VERY BEGINNING of processing any command:
   - Parse user instructions and append new requirement items to `docs/check-list.md` formatted as `- [X] [Requirement Description]` (Include Traditional Chinese).
   - **Purge Completed Items**: Check existing items in `docs/check-list.md`. Any item manually marked by human as `[O]` MUST be completely removed from the file so that ONLY active `[X]` items remain.
...
4. **Persistent Execution**: As long as there are unresolved `[X]` items, the Agent MUST prioritize implementing and completing those items in the current command execution, unless the user explicitly states to abandon or remove the requirement.
...
1. Audit `docs/check-list.md` for conflicts and pending `[X]` items (and prune `[O]` items). Ensure `.gitignore` excludes `docs/check-list.md`.
...
6. Prior to executing `git add`, reconcile `.env.example`, append to `docs/change-log.md` with exact timestamp, update and convert `docs/system-spec.md` (Bilingual) and root `README.md` (Bilingual), and verify all 3 mandatory dynamic docs exist in `docs/`.

---

## 8. 📦 Pre-Git-Add Staging Specification

Prior to ANY `git add` or staging action, ensure BOTH the root **`README.md`** AND **`docs/system-spec.md`** are completely synchronized, updated in BILINGUAL format (**English & Traditional Chinese**), and include:
1. **Project Manifesto & Overview** (Sync with `docs/system-spec.md` -> Overview)
2. **Core Capabilities** (Sync with `docs/system-spec.md` -> Features)
3. **Architecture & Data Flow** (Sync with `docs/system-spec.md` -> Architecture)
4. **Engineering Principles & Author Info** (Author: HSU, CHE-CHENG 許哲誠, SOLID, Anti-Hallucination, Strict IP, Security, Scope Isolation, RWD, Checklist Conflict Protocol)
5. **Technology Stack Matrix** (Sync with `docs/system-spec.md` -> Tech Stack)
6. **Directory Structure Tree**
7. **Getting Started & `.env` Configuration**
