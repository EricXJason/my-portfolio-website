...
## 4. 📦 Pre-Git-Add 暫存前置規範與 README 結構要求

本章節規範執行任何 `git add` 進入暫存區之前，說明文件必須落實的標準審查與結構細節。

### 4.1 雙語 README 同步要求
於執行任何 `git add` 前，必須確保根目錄之 **`README.md`** 已完全同步並更新為**精確的中英雙語對照 (English & Traditional Chinese)**。

### 4.2 README 四大核心結構審查
根目錄 `README.md` 必須強制具備以下 4 大核心結構，其餘章節得依專案彈性選配，嚴禁空殼：
1. **專案簡介 (Project Overview)**: 專案摘要、願景、核心價值與作者資訊（專案作者: 許哲誠 HSU, CHE-CHENG）。
2. **技術棧 (Technology Stack)**: 核心框架、語言版本、相依套件庫矩陣。
3. **目錄結構 (Directory Structure)**: 符合當前語言規範之目錄樹與各層職責簡介。
4. **本地開發配置 (Local Development Setup)**: 前置需求、環境變數配置、安裝步驟與啟動除錯指令。
