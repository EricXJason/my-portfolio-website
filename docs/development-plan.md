...
4. **專案作品 (Projects)**：
   - **104 高階技術履歷標準**：全站 14 件專案均採軟體架構思維與 STAR 原則（情境 Situation、任務 Task、行動 Action、成果 Result）撰寫，消除空泛展示性語調。
   - **雙語國際化 (i18n) 深度架構**：以「個人作品集網站」為核心示範自主研發之 Context-based i18n 輕量化系統，全站專案資料集（`projects-section.json`）完整具備 `title_zh` / `title_en`、`desc` / `desc_en`、`contributions` / `contributions_en`、`honors` / `honors_en` 與 `date` / `date_en` 之零遺漏雙語字典映射與 0ms 響應式切換。
   - **類別過濾系統**：精選作品、全部作品、互動應用（7件）、前端開發（3件）、全端開發（4件）。
   - **精選作品視圖**：3 件核心代表作橫向對稱卡片，支援「檢視更多」快速跳轉。
   - **全部作品視圖**：14 件專案之完整卡片矩陣，涵蓋已實作專案之專業技術貢獻與籌備中專案（In Development）之規劃狀態與雙語對照。
   - **專案詳情燈箱彈窗**：寬螢幕雙欄橫向佈局，展示技術棧、專案貢獻亮點、榮譽獎項、演示影片與 GitHub / 線上網站外鏈。
   - **YouTube 影音展示彈窗**：支援專案展示影片之即時嵌入播放。
