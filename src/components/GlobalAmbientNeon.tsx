...
export const GlobalAmbientNeon: React.FC = () => {
  ...
  // Subtle & Symmetrical Atmosphere Colors (Dark vs Light - Slightly Boosted Sweet Spot)
  const leftCyanOuter = isLight ? 'rgba(56, 189, 248, 0.13)' : 'rgba(0, 240, 255, 0.11)';
  const leftBlueOuter = isLight ? 'rgba(2, 132, 199, 0.11)' : 'rgba(14, 165, 233, 0.09)';
  const leftCyanCore  = isLight ? 'rgba(2, 132, 199, 0.19)' : 'rgba(0, 240, 255, 0.17)';

  const rightPurpleOuter = isLight ? 'rgba(192, 132, 252, 0.13)' : 'rgba(168, 85, 247, 0.11)';
  const rightVioletOuter = isLight ? 'rgba(124, 58, 237, 0.11)' : 'rgba(147, 51, 234, 0.09)';
  const rightPurpleCore  = isLight ? 'rgba(147, 51, 234, 0.19)' : 'rgba(168, 85, 247, 0.17)';
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      {/* ── LEFT SIDE AMBIENT NEON SYSTEM (CYAN / ELECTRIC BLUE — INSTANT 0MS UNIFIED) ── */}

      {/* Symmetrical Vertical Edge Aura Strip - Left */}
      <div
        className="absolute top-0 bottom-0 left-0 w-28 sm:w-48 opacity-40 pointer-events-none animate-ambient-core-pulse"
        style={{
          background: isLight
      ...
      <div
        ...
        style={{ backgroundColor: leftBlueOuter }}
      >
        <div
      ...
      {/* ── RIGHT SIDE AMBIENT NEON SYSTEM (PURPLE / VIOLET — INSTANT 0MS UNIFIED) ── */}

      {/* Symmetrical Vertical Edge Aura Strip - Right */}
      <div
        className="absolute top-0 bottom-0 right-0 w-28 sm:w-48 opacity-40 pointer-events-none animate-ambient-core-pulse"
        style={{
          background: isLight
      ...
      <div
        ...
        style={{ backgroundColor: rightVioletOuter }}
      >
        <div
          className="absolute inset-24 rounded-full blur-[110px] animate-ambient-core-pulse"
          style={{ backgroundColor: rightPurpleCore }}
