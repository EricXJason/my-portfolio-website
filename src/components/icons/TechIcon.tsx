 ...
 * ============================================================================
 * 檔案名稱: TechIcon.tsx
 * 所屬模組: Core UI System (技術標誌光學等大正規化渲染組件)
 * 責任描述: 負責渲染專案 12 大技術 SVG 向量標誌，並透過精密光學尺寸補償矩陣消除不同幾何形狀之視覺面積不均。
 * 架構分層: Shared UI Component Layer
 * 依賴關係: 依賴 public/tech-icons/ 實體 SVG 與 src/data/tech-icons.json 向量庫。
 * 邊界處理: 100% 本地實體 SVG 零外部 CDN 依賴、未知名稱備援保護、等比 flexbox 居中容器防止破版。
 * ============================================================================
 */
...
/**
 * 光學尺寸補償矩陣 (Optical Sizing Scale Factors)
 * 解決不同幾何形狀在相同像素尺寸下的「視覺重量與視覺面積不均」問題：
 * - 實心正方形 (TypeScript, JavaScript)：填滿面積 100%，肉眼感覺過於巨大，縮小至 0.84
 * - 盾牌與標準形 (HTML, CSS)：色塊飽滿垂直高度頂天立地，微調至 0.90
 * - 實心尖頂火焰 (Firebase)：色塊飽滿垂直高度大，微調至 0.88
 * - 粗線條多孔 (GitHub Actions)：節點外擴面積大，微調至 0.88
 * - 扁平橫長形 (Cloudflare Pages)：微調至 1.08
 * - 橫向扁平小水波 (Tailwind CSS)：高度極矮小，補償放大至 1.25 達到等同視覺高度
 * - 鏤空細線多孔 (React)：三環細原子軌道內部空心，補償至 1.16 充實視覺重量
 * - 尖錐閃電 (Vite)：維持 0.98
 */
const getOpticalScale = (key: string): number => {
  if (key.includes('typescript') || key === 'ts' || key.includes('javascript') || key === 'js') {
    return 0.84;
  }
  ...
  if (key.includes('cloudflare') || key === 'cf') {
    return 1.08;
  }
  if (key.includes('firebase')) {
    return 0.88;
  }
  if (key.includes('githubaction')) {
    return 0.88;
  }
  if (key.includes('html') || (key.includes('css') && !key.includes('tailwind'))) {
    return 0.90;
  }
...
export const TechIcon: React.FC<TechIconProps> = ({
  ...
  const opticalScale = getOpticalScale(nKey);
  const innerSize = Math.max(1, Math.round(size * opticalScale));

  // 內部渲染各技術的原生向量 SVG
  const renderSvg = () => {
    // 1. Vite SVG (from public/tech-icons/vite.svg)
    ...
    // 2. React SVG (from public/tech-icons/react.svg)
    ...
    // 3. TypeScript SVG (from public/tech-icons/typescript.svg)
    ...
    // 4. JavaScript SVG (from public/tech-icons/javascript.svg)
    ...
    // 5. HTML SVG (from public/tech-icons/html.svg)
    ...
    // 6. CSS SVG (from public/tech-icons/css.svg)
    ...
    // 7. Tailwind CSS SVG (from public/tech-icons/tailwind-css.svg)
    ...
    // 8. GitHub Actions SVG (from public/tech-icons/github-actions.svg)
    ...
    // 9. GitHub SVG (from public/tech-icons/github.svg)
    ...
    // 10. ArtStation SVG (from public/tech-icons/artstation.svg)
    ...
    // 11. Cloudflare / Cloudflare Pages SVG (經 viewBox 與高寬比重構，徹底消除上下多餘留白)
    if (nKey.includes('cloudflare') || nKey === 'cf') {
      return (
        <svg
          width={innerSize}
          height={Math.round(innerSize * 0.72)}
          viewBox="0 0 256 120"
          xmlns="http://www.w3.org/2000/svg"
          className="inline-block shrink-0"
        >
    ...
    // 12. Firebase SVG (from public/tech-icons/firebase.svg — 官方三層火焰多邊形)
    if (nKey.includes('firebase')) {
      return (
        <svg
          width={innerSize}
          height={innerSize}
        ...
        >
          ...
          <path d="M1.253 280.732l1.605-3.131 99.353-188.518-44.15-83.475C54.392-1.283 45.074.474 43.87 8.188L1.253 280.732z" fill="#FFA000" />
          ...
          <path d="M134.417 148.974l32.039-32.812-32.039-61.007c-3.042-5.791-10.433-6.398-13.443-.59l-17.705 34.109-.53 1.744 31.678 58.556z" fill="#F57C00" />
          <path d="M0 282.998l.962-.968 3.496-1.42 128.477-128 1.628-4.431-32.05-61.074z" fill="#F6820C" />
          ...
          <path d="M254.354 282.16L221.402 79.218c-1.03-6.35-7.558-8.977-12.103-4.424L1.29 282.6l114.339 63.908a23.943 23.943 0 0 0 23.334.006l115.392-64.355z" fill="#FFCA28" />
